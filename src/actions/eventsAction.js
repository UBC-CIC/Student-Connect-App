    ...
    lambda.invoke(params, function (err, data) {
      ...
      // an error occurred
        ...
        results.map((item) => {
          ...
          if (item._source.excerpt)
            item._source.excerpt = htmlDecoder(item._source.excerpt);

          if (item._source.startDate)
            item._source.startDate = eventDateCleaner(item._source.startDate);
          if (item._source.endDate)
            item._source.endDate = eventDateCleaner(item._source.endDate);
          if (item._source.endDate)
            item._source.endDate = eventEndDateCleaner(
              item._source.startDate,
              item._source.endDate
            );
          if (item._source.title)
            item._source.title = htmlDecoder(item._source.title);
          if (item._source.eventLocation.venue)
            item._source.eventLocation.venue = htmlDecoder(
              item._source.eventLocation.venue
            );
          if (item._source.eventLocation)
            item._source.eventLocation.venue = eventLocationCleaner(
              item._source.eventLocation
            );
        });
...
export const fetchAllEvents = () => {
  return async (dispatch) => {
    ...
    fetchedData.map((item) => {
      ...
      if (item.endDate)
        item.endDate = eventEndDateCleaner(item.startDate, item.endDate);
      if (item.title) item.title = htmlDecoder(item.title);
      if (item.excerpt) item.excerpt = htmlDecoder(item.excerpt);
      if (item.eventLocation.venue)
        item.eventLocation.venue = htmlDecoder(item.eventLocation.venue);
      if (item.eventLocation)
        item.eventLocation.venue = eventLocationCleaner(item.eventLocation);
    ...
    dispatch(fetchAllEventsSuccess(fetchedData));
  };
};
