# Needs access to query params, ddb? s3 bucket?
# API_STUDENTENGAGEMENT_ATHLETICSNEWSTABLE_ARN
# API_STUDENTENGAGEMENT_ATHLETICSNEWSTABLE_NAME
...
ATHLETICS_NEWS_TABLE = os.environ["API_STUDENTENGAGEMENT_ATHLETICSNEWSTABLE_NAME"]
...
athletics_code_map = {
    ...
    "mrugby": ["Male", "Rugby", "Sports"],
    "sball": ["Female", "Softball", "Sports"],
    "ultimate": ["Ultimate", "Sports"]
}
...
def news_parser(news_json):
    ...
    if item_datetime == "Null":
        item_datetime = str(datetime.now(
...
def handler(event, context):
    ...
    for item in response_items:
        try:
            ...
            LOGGER.error(
                f"Error in parsing a news item, raw item saved to {S3_BUCKET_NAME}/ErrorLog/AthleticsNews")
            detailed_exception(LOGGER)

    # Filter the parsed items based on last query time to get only new items
    try:
        last_query_time = SSM_CLIENT.get_parameter(Name="AthleticsNewsQueryTime")[
            "Parameter"]["Value"]
