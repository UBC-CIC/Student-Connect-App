# API_STUDENTENGAGEMENT_CLUBSTABLE_ARN
# API_STUDENTENGAGEMENT_CLUBSTABLE_NAME
...
CLUBS_TABLE = os.environ["API_STUDENTENGAGEMENT_CLUBSTABLE_NAME"]
    ...
    for node in club_html_nodes:
        ...
        if len(paragraphs) == 0:
            ...
            for index, paragraph in enumerate(paragraphs):
                club_item["description"] = club_item["description"] + \
        ...
        for link in link_list:
            ...
            if link_name == "email":
                ...
                # Check to ignore broken Facebook links
            ...
            for club_link in club_links:
                if club_link.attrs['href'] == "/health-dental" \
                    ...
                    try:
                        ...
                        club_html_nodes.append(club_soup.find(
                            "div", {"class": "club-item-content"}))
                    except RequestException as e:
                        LOGGER.error(
                            f"Network request for club:{club_page_url} failed")
                        print("Request Exception")
        except RequestException as e:
            LOGGER.error(
                f"Network request for clubs under letter:{letter} failed")
            print("Request Exception")
...
def get_course_unions(url: str):
    ...
    for letter in letter_list:
        ...
        node = driver.find_element_by_xpath(
            '//*[@id="block-views-club-directory-block-2"]/div/div')
...
def handler(event, context):
    ...
    response = s3_client.get_object(
        Bucket=S3_BUCKET_NAME, Key="AllUBCOClubs.json")
    ...
    for club in clubs_json:
        ...
        club["documentId"] = hashlib.md5(
            str(club["title"]).encode("utf-8")).hexdigest()
